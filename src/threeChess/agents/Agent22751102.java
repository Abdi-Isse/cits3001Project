...
public class Agent22751102 extends Agent {
    ...
    public Position[] playMove(Board board) {
        ...
        if (moveCount > 0) {
            ...
            try {
                Position start = Position.get(Colour.values()[(lastMove[0].getColour().ordinal() + 1) % 3],
                        lastMove[0].getRow(), lastMove[0].getColumn());
                Position end = Position.get(Colour.values()[(lastMove[1].getColour().ordinal() + 1) % 3],
                        lastMove[1].getRow(), lastMove[1].getColumn());
